...
func NewDatabase(connectionString string) (*Database, error) {
	...
	if err := database.ensureTables(); err != nil {
		fmt.Printf("Warning: failed to ensure tables: %v\n", err)
	}

	return database, nil
}

func (db *Database) ensureTables() error {
	migrations := []string{
...
func (db *Database) CreateDocument(doc *models.Document) error {
	query := `
		INSERT INTO documents (id, user_id, filename, original_filename, file_path, file_size, status)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
	`
	_, err := db.conn.Exec(query, doc.ID, doc.UserID, doc.Filename, doc.OriginalFilename, doc.FilePath, doc.FileSize, doc.Status)
	return err
}

func (db *Database) GetDocument(id, userID string) (*models.Document, error) {
	query := `
...
func (db *Database) ListDocuments(userID string, limit, offset int) ([]models.Document, error) {
	query := `
		SELECT id, filename, original_filename, file_size, pdf_type,
		       status, error_message, created_at, updated_at, processed_at
		FROM documents
		WHERE user_id = $1
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&doc.ID, &doc.Filename, &doc.OriginalFilename, &doc.FileSize,
			&doc.PDFType, &doc.Status, &doc.ErrorMessage, &doc.CreatedAt, &doc.UpdatedAt, &doc.ProcessedAt,
		)
		if err != nil {
			return nil, err
...
func (db *Database) DeleteDocument(id, userID string) error {
...
func (db *Database) SaveChatMessage(msg *models.ChatMessage) error {
	...
	query := `
		INSERT INTO chat_history (id, user_id, document_id, session_id, role, message)
		VALUES ($1, $2, $3, $4, $5, $6)
	`
	_, err := db.conn.Exec(query, msg.ID, msg.UserID, docID, msg.SessionID, msg.Role, msg.Message)
	return err
}

func (db *Database) GetChatHistory(documentID, sessionID, userID string, limit int) ([]models.ChatMessage, error) {
	var query string
	var rows *sql.Rows
	var err error
	...
	if sessionID != "" {
		query = `
			SELECT ch.id, ch.document_id, ch.session_id, ch.role, ch.message, ch.created_at
			FROM chat_history ch
			JOIN chat_sessions cs ON ch.session_id = cs.id
			WHERE ch.session_id = $1 AND cs.user_id = $2
			ORDER BY ch.created_at ASC
			LIMIT $3
		`
		rows, err = db.conn.Query(query, sessionID, userID, limit)
	} else {
		query = `
...
func (db *Database) CreateChatSession(session *models.ChatSession) error {
	...
	query := `
		...
		VALUES ($1, $2, $3, $4)
	`
	_, err := db.conn.Exec(query, session.ID, session.UserID, docID, session.Title)
	return err
}

func (db *Database) GetChatSession(id, userID string) (*models.ChatSession, error) {
	query := `
...
func (db *Database) ListChatSessions(userID string, limit, offset int) ([]models.ChatSession, error) {
	query := `
		SELECT id, document_id, title, created_at, updated_at
		FROM chat_sessions
		WHERE user_id = $1
...
func (db *Database) DeleteChatSession(id, userID string) error {
	query := `DELETE FROM chat_sessions WHERE id = $1 AND user_id = $2`
	_, err := db.conn.Exec(query, id, userID)
	return err
}

func (db *Database) DeleteDocumentChatSessions(documentID, userID string) error {
	query := `DELETE FROM chat_sessions WHERE document_id = $1 AND user_id = $2`
	_, err := db.conn.Exec(query, documentID, userID)
	return err
}

func (db *Database) GetDocumentStatusCounts(userID string) (map[string]int, error) {
	query := `
...
// GetDocumentByID returns a document without user filter (for internal/AI service use)
func (db *Database) GetDocumentByID(id string) (*models.Document, error) {
	query := `
		SELECT id, filename, original_filename, file_path, file_size, pdf_type,
		       status, error_message, created_at, updated_at, processed_at
		FROM documents
